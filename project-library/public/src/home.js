function getTotalBooksCount(books) {
  // YOUR SOLUTION HERE
  // Use the reduce method to accumulate the total count of books
  const totalCount = books.reduce((count, book) => {
    // Increment the count by 1 for each book
    return count + 1;
  }, 0);
...
function getBooksBorrowedCount(books) {
  ...
  // Filter the books array to get borrowed books
  const borrowedBooks = books.filter(book => !book.borrows[0].returned);
  
  // Return the length of the borrowedBooks array
